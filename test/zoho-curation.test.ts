...
async function loadZohoPayload(): Promise<ZohoPayload> {
  const filePath = path.resolve(process.cwd(), "data/processed/zoho-crm-js-sdk-snippets.json");
  return JSON.parse(await fs.readFile(filePath, "utf8")) as ZohoPayload;
}

test("zoho curation schema v1 exists and has required coverage", async () => {
  const payload = await loadZohoPayload();

  assert.equal(payload.schemaVersion, "zoho-crm-js-sdk-kb/v1");
  assert.ok(payload.summary.rawUnits >= 102);

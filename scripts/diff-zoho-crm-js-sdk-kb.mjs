...
export function buildDiffReport(oldPayload, newPayload) {
  ...
  return {
    ...
    schemaVersion: "zoho-crm-js-sdk-diff/v1",
    counts: {
    ...
    reviewReason: requiresManualReview
      ? "Tier A canonical knowledge changed. Manual curation review is required before promotion."
...
function toMarkdown(report) {
  const lines = [];
  lines.push("# Zoho CRM JS SDK KB Diff Report");
  lines.push("");
...
async function main() {
  const oldPath = process.argv[2] ?? "data/processed/zoho-crm-js-sdk-snippets.old.json";
  const newPath = process.argv[3] ?? "data/processed/zoho-crm-js-sdk-snippets.json";
  const outJson = process.argv[4] ?? "data/reports/zoho-crm-js-sdk-diff-report.json";
  const outMd = process.argv[5] ?? "data/reports/zoho-crm-js-sdk-diff-report.md";
